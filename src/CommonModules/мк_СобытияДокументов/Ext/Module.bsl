...
//	См. "ДокументОбъект.ПередЗаписью" в синтакс-помощнике.
//
// Параметры:
...
Процедура ПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если ПараметрыОбработчиков.ПередЗаписью Тогда
		Модуль.ПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения);	
	КонецЕсли;	
...
// Процедура - обработчик подписки на событие "ПриКопированиии" для Документов.
//	См. "ДокументОбъект.ПриКопировании" в синтакс-помощнике.
//
// Параметры:
//  Источник  - ДокументОбъект - Источник события.
//  ОбъектКопирования  - ДокументОбъект - Объект копирования.
//
Процедура ПриКопировании(Источник, ОбъектКопирования) Экспорт
	...
	Если ПараметрыОбработчиков.ПриКопировании Тогда
		Модуль.ПриКопировании(Источник, ОбъектКопирования);
	КонецЕсли;	
	
КонецПроцедуры

// Процедура - обработчик подписки на событие "ПриУстановкеНовогоНомера" для Документов.
//	См. "ДокументОбъект.ПриУстановкеНовогоНомера" в синтакс-помощнике.
//
// Параметры:
//  Источник  - ДокументОбъект - Источник события.
...
Процедура ПриУстановкеНовогоНомера(Источник, СтандартнаяОбработка, Префикс) Экспорт
	...
	Если ПараметрыОбработчиков.ПриУстановкеНовогоНомера Тогда
		Модуль.ПриУстановкеНовогоНомера(Источник, СтандартнаяОбработка, Префикс);
	КонецЕсли;	
...
//
Функция ПараметрыОбработчиковПоУмолчанию() Экспорт
	...
	ПараметрыОбработчиков.Вставить("ПриУстановкеНовогоНомера", Ложь);
...
Функция ПараметрыОбработчиковОбъекта(Модуль)
	...
	ПараметрыОбработчиковПоУмолчанию = ПараметрыОбработчиковПоУмолчанию();
	
	Если Модуль = Неопределено Тогда
		Возврат ПараметрыОбработчиковПоУмолчанию;
	КонецЕсли;	
		
	Попытка 
		// На случай, если в заданном модуле нет такой функции.
		ПараметрыОбработчиков = Модуль.ПараметрыОбработчиков();
	Исключение
...
// 	имя формы "Справочник.Номенклатура.Форма.ФормаЭлемента", модуль - "Форма_Справочник_Номенклатура_ФормаЭлемента".
//
// Параметры:
...
Функция ОбщийМодульОбъекта(Источник)
	...
	Модуль = мк_ОбщегоНазначения.ОбщийМодульПереопределяемогоОбъекта(ИмяОбъекта);
